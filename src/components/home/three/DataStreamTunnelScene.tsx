...
const DataParticle = ({ position }: { position: THREE.Vector3 }) => {
    ...
    useFrame((state) => {
        if(ref.current) {
            // FIX: Use .setZ instead of direct property mutation to avoid 'read only property' error
            const newZ = ref.current.position.z + 0.1;
            if (newZ > 5) {
                ref.current.position.setZ(-20);
            } else {
                ref.current.position.setZ(newZ);
            }
        }
    });
    return (
