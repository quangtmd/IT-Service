...
const TechImage: React.FC<{ url: string; scale: [number, number]; position: [number, number, number] }> = ({ url, scale, position }) => {
  return (
...
const Scene = () => {
    ...
    const imageData = [
        { url: 'https://images.unsplash.com/photo-1498050108023-c5249f4df085?q=80&w=1200', position: [1, -h * 0.2, -4], scale: [w / 3.5, w / 5] as [number, number] },
        { url: 'https://images.unsplash.com/photo-1555774698-0b77e0abfe3d?q=80&w=1200', position: [-1.2, -h * 0.8, -3], scale: [w / 3, w / 4] as [number, number] },
        { url: 'https://images.unsplash.com/photo-1587620962725-abab7fe55159?q=80&w=1200', position: [1.5, -h * 1.5, -3.5], scale: [w / 3.5, w / 3.5] as [number, number] },
        { url: 'https://images.unsplash.com/photo-1542744173-8e7e53415bb0?q=80&w=1200', position: [-1, -h * 2.3, -5], scale: [w / 2.5, w / 4] as [number, number] },
        { url: 'https://images.unsplash.com/photo-1605379399642-870262d3d051?q=80&w=1200', position: [1.2, -h * 3.0, -3], scale: [w / 2.8, w / 2.8] as [number, number] },
    ];
    ...
    return (
        <group ref={group}>
            ...
            {imageData.map((img, i) => (
                <TechImage key={i} url={img.url} scale={img.scale} position={img.position as [number, number, number]} />
            ))}
