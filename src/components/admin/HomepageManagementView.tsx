...
const HomepageManagementView: React.FC = () => {
    ...
    const renderContent = () => {
        // FIX: Ensure fallbacks to INITIAL constants are used if specific sections are missing from settings
        // This prevents "Cannot read properties of undefined" errors when iterating over items/features.
        switch (activeTab) {
            case 'banners': return <BannerSettingsForm banners={settings.homepageBanners || Constants.INITIAL_HOMEPAGE_BANNERS} onChange={(val) => handleSectionChange('homepageBanners', val)} />;
            ...
            case 'about': return <AboutSettingsForm about={settings.homepageAbout || Constants.INITIAL_HOMEPAGE_ABOUT} onChange={(val) => handleSectionChange('homepageAbout', val)} />;
            case 'services_benefits': return <ServicesBenefitsSettingsForm settings={settings.homepageServicesBenefits || Constants.INITIAL_HOMEPAGE_SERVICES_BENEFITS} onChange={(val) => handleSectionChange('homepageServicesBenefits', val)} />;
            case 'why_choose_us': return <WhyChooseUsSettingsForm settings={settings.homepageWhyChooseUs || Constants.INITIAL_HOMEPAGE_WHY_CHOOSE_US} onChange={(val) => handleSectionChange('homepageWhyChooseUs', val)} />;
            case 'stats_counter': return <StatsCounterSettingsForm settings={settings.homepageStatsCounter || Constants.INITIAL_HOMEPAGE_STATS_COUNTER} onChange={(val) => handleSectionChange('homepageStatsCounter', val)} />;
            case 'testimonials': return <TestimonialsSettingsForm settings={settings.homepageTestimonials || Constants.INITIAL_HOMEPAGE_TESTIMONIALS} onChange={(val) => handleSectionChange('homepageTestimonials', val)} />;
            case 'featured_projects': return <FeaturedProjectsSettingsForm settings={settings.homepageFeaturedProjects || Constants.INITIAL_HOMEPAGE_FEATURED_PROJECTS} onChange={(val) => handleSectionChange('homepageFeaturedProjects', val)} />;
            case 'brand_logos': return <BrandLogosSettingsForm settings={settings.homepageBrandLogos || Constants.INITIAL_HOMEPAGE_BRAND_LOGOS} onChange={(val) => handleSectionChange('homepageBrandLogos', val)} />;
            case 'process': return <ProcessSettingsForm settings={settings.homepageProcess || Constants.INITIAL_HOMEPAGE_PROCESS} onChange={(val) => handleSectionChange('homepageProcess', val)} />;
            case 'cta': return <CallToActionSettingsForm settings={settings.homepageCallToAction || Constants.INITIAL_HOMEPAGE_CALL_TO_ACTION} onChange={(val) => handleSectionChange('homepageCallToAction', val)} />;
            case 'blog_preview': return <BlogPreviewSettingsForm settings={settings.homepageBlogPreview || Constants.INITIAL_HOMEPAGE_BLOG_PREVIEW} onChange={(val) => handleSectionChange('homepageBlogPreview', val)} />;
            case 'contact': return <ContactSectionSettingsForm settings={settings.homepageContactSection || Constants.INITIAL_HOMEPAGE_CONTACT_SECTION} onChange={(val) => handleSectionChange('homepageContactSection', val)} />;
            default: return null;
        }
    };
