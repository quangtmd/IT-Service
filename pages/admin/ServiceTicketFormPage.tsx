    ...
    useEffect(() => {
        const loadData = async () => {
            if (isEditing) {
                ...
                setError(null);
                try {
                    // In a real app, this would be a single API call: getServiceTicket(ticketId)
                ...
                <div className="admin-card-body admin-product-form-page-body">
                    ...
                    <div className="overflow-x-auto">
                         <table className="admin-table w-full text-xs">
                             <thead>
                                <tr>
                                    <th>Mã TB</th><th>Tên TB</th><th>Nội dung</th><th>SL</th><th>Giá</th><th>Chi phí DK</th><th>Hành động</th>
                                </tr>
                             </thead>
                             <tbody>
                                {(formData.details || []).map((detail, index) => (
                                    <tr key={index}>
                                        ...
                                        <td><Button type="button" size="sm" variant="ghost" className="!text-red-500" onClick={() => removeDetailItem(index)}><i className="fas fa-trash"></i></Button></td>
                                    </tr>
                                ))}
                             </tbody>
                         </table>
                    </div>
                     <Button type="button" size="sm" variant="outline" onClick={addDetailItem} leftIcon={<i className="fas fa-plus"></i>} className="mt-2">Thêm dòng</Button>
                </div>
            </form>
        </div>
    );
